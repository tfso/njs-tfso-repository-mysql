...
class QueryStream extends query_1.Query {
    ...
    executeQuery() {
        return new Promise((resolve, reject) => {
            try {
                let query, parameters = {}, timed = Date.now(), error = null, records = [], predicate, totalRecords = -1, affectedRecords = 0, changedRecords = 0, cancelled = false, completed = false;
                ...
                query.on('end', () => {
                    if (error != null)
                        ...
                        resolve(new recordset_1.RecordSet(records, changedRecords || affectedRecords, (Date.now() - timed), undefined));
                });
            }
            catch (ex) {
                reject(ex);
